...
- **Standalone collection mapping methods (v1.5+).** Dedicated collection-level forge methods (`IEnumerable<TDest> ForgeAll(IEnumerable<TSrc>)`) are auto-discovered from element-level forge methods. This simplifies collection mapping at call sites — instead of `.Select(x => forger.Forge(x))`, call `forger.ForgeAll(items)` directly. FM0041 reports missing element mappings and FM0042 reports ambiguous matches.
