...
- **`[ConvertWith]` delegates to `ITypeConverter<TSource, TDest>` implementations (v1.4+).** Apply `[ConvertWith(typeof(MyConverter))]` to a forge method to delegate the entire conversion to the converter's `Convert` method. Also supports member-based references via `[ConvertWith(nameof(_field))]` for DI-injected converters. The generator resolves the converter instance in order: `IServiceScopeFactory` field → `IServiceProvider` field → field/property reference → parameterless constructor. When `[ConvertWith]` is present, it takes full precedence over `[ForgeProperty]`/`[ForgeFrom]`/`[ForgeWith]` (FM0036 warning).
...
- **String↔enum auto-conversion (v1.4+, default on).** When source property is `string` and destination is `enum` (or vice versa), the generator auto-converts using `Enum.Parse` (throws on invalid) by default. Control per-forger via `[ForgeMap(StringToEnum = StringToEnumConversion.TryParse)]` (falls back to `default(T)`) or `StringToEnumConversion.None` (requires explicit `[ForgeFrom]`). Assembly default via `[ForgeMapDefaults(StringToEnum = ...)]`. All nullable combinations (`string?`↔`enum?`) are supported, respecting `NullPropertyHandling`. Reverse via `[ReverseForge]` emits `.ToString()`.
- **Nested existing-target mapping (v1.4+, opt-in).** `[ForgeProperty("Prop", "Prop", ExistingTarget = true)]` updates nested objects in place instead of replacing them — critical for EF Core change tracking. Only valid on `[UseExistingValue]` mutation methods (FM0028). For collections, use `CollectionUpdate = CollectionUpdateStrategy.Sync` with `KeyProperty = "Id"` to match, update, add, and remove elements by key (FM0031/FM0032). Sync requires: a `ForgeInto` method for updating matched items, a `Forge` method for adding new items (FM0030), and a `List<T>` destination (other collection types are not supported).
