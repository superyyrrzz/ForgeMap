...
- **Standalone collection mapping methods (v1.5+).** Declare a partial method with collection parameter and return types, and the generator auto-discovers the matching element-level forge method to implement the body. This provides a typed, materializing alternative to manual `.Select(x => forger.Forge(x)).ToList()` calls — call `forger.ForgeAll(items)` instead. Note: unlike lazy `Select`, standalone methods eagerly materialize into the declared return type. FM0041 reports missing element mappings and FM0042 reports ambiguous matches.
