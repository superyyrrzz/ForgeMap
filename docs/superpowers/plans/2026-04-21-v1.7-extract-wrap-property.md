...
- `ForgeCodeEmitter.GenerateMethod` (file `src/ForgeMap.Generator/ForgeCodeEmitter.cs`, lines 296–424) is the central method-shape dispatcher. It currently routes in order: ForgeInto → ConvertWith → enum forge → collection forge → object forge. Feature 3 inserts a new branch immediately *before* ConvertWith (so the Extract/Wrap branch can diagnose FM0065 conflicts when `[ConvertWith]`/`[ForgeFrom]`/`[ForgeWith]`/`[ForgeProperty]` are also present, and so `[ExtractProperty]`/`[WrapProperty]` cannot accidentally be misclassified as collection forges).
